...
if [[ "$SCRIPT_DIR" == *"/.devtools/scripts"* ]]; then
  # Running from .devtools submodule
...
compose_files=(
  ...
  "$BASE_DIR/docker-compose.dev.yaml"
)

if [ "$DB" = "postgres" ]; then
  compose_files+=(
    "$BASE_DIR/docker-compose.postgresql.yml"
    "$BASE_DIR/docker-compose.postgresql.dev.yml"
  )
  DB_SERVICE="postgres"
else
  compose_files+=(
    "$BASE_DIR/docker-compose.mariadb.yml"
    "$BASE_DIR/docker-compose.mariadb.dev.yml"
  )
  DB_SERVICE="mariadb"
fi

compose_files+=(
  "$BASE_DIR/docker-compose.adminer.yml"
  "$BASE_DIR/docker-compose.phpmyadmin.yml"
)
